...
function App() {
    //running on kovan
  const lotteryAddress = "0xACe703fc096F2fCdc4c2eA0A3a2C99871fEFBca7"
  ...
  const [account, setAccount] = useState("")
   // const [chainID, setChainID] = useState("")
    ...
    const eventContract = new ethers.Contract(lotteryAddress, lotteryABI.abi, provider);
...
useEffect(() =>{
  eventContract.on("lotteryEnd", (winnerAddressEvent,           totalCurrentPoolEvent, timeEvent) => 
      getWinnerAddress())
  return () => {
      eventContract.removeListener("lotteryEnd", (winnerAddressEvent, totalCurrentPoolEvent, timeEvent) => 
      getWinnerAddress())
  }
},[])
...
//eventContract.on('newParticipant', handleParticipantArray);

//function handleParticipantArray(_chainId) {
...
//}
    ...
    async function enterPoolContract(){
        ...
        let data = await contract.enterPool({value: ethers.utils.parseEther(price.toString())})
        await data.wait()
        getContractParticipantsArray()
        getCurrentPool()
    }
    ...
    const [currentUnix, setCurrentUnix] = useState(Math.round(new Date().getTime()/1000))

    function getCurrentUnixTime(){
        setCurrentUnix(Math.round(new Date().getTime()/1000))
        if(currentUnix >= endTime && lengthPlayerArray >= 2 || lotteryBool == false){
  ...
  return (
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css" integrity="sha512-Fo3rlrZj/k7ujTnHg4CGR2D7kSs0v4LLanw2qksYuRlEzO+tcaEPQogQ0KaoGN26/zrn20ImR1DfuLWnOo7aBA==" crossOrigin="anonymous" referrerPolicy="no-referrer" />
      <header className="App-header">
       ...
       <Routes>
          <Route exact path="/"  element={<Home isLotteryRunning={isLotteryRunning} startTime={startTime} getLotteryEndingTime={getLotteryEndingTime} getCurrentPool={getCurrentPool} currentPool={currentPool} getTime={getTime} time={time} getPrice={getPrice} price={price} getWinnerAddress={getWinnerAddress} winner={winner} enterPoolContract={enterPoolContract} getContractParticipantsArray={getContractParticipantsArray} playerArray={playerArray}/>} />

          <Route exact path="/Management" element={<Management lotteryAddress={lotteryAddress} changingTimeInterval={changingTimeInterval} handleChange={handleChange} getOwner={getOwner} owner={owner} chooseWinnerContract={chooseWinnerContract} startNewLotteryContract={startNewLotteryContract} changeEntryPrice={changeEntryPrice} handleChangePrice={handleChangePrice} getContractBalance={getContractBalance} balance={balance} withdrawContractProfits={withdrawContractProfits} getContractProfits={getContractProfits} lotteryProfits={lotteryProfits} getWinnerAddress={getWinnerAddress} winner={winner}/>} />

          <Route exact path="/Account" element={<Account 
          account={account} networkchainId={network.chainId} networkname={network.name} withdrawPriceContract={withdrawPriceContract} handleChangeWithdraw={handleChangeWithdraw} getPersonalWinnings={getPersonalWinnings} addrFunds={addrFunds} handleChangeAddr={handleChangeAddr} />} />
        </Routes>
            </div>
